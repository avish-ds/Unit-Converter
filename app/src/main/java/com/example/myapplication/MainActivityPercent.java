...
public class MainActivityPercent extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v){
        ...
        if(v.equals(bconvert)){
            try {
                Float numerator =Float.parseFloat(txtFrom.getText().toString());
                Float denominator =Float.parseFloat(txtTo.getText().toString());
                Float percent=(float)((numerator/denominator)*100);
                String output=String.valueOf(percent)+"%";

                makeText(getBaseContext(),output,Toast.LENGTH_LONG).show();
